...
class ModelTrainer:
    ...
    def __init__(self, model_name: str = "meta-llama/Llama-4-Scout", output_dir: str = "./llama4_iomt_model"):
        ...
        # load tokenizer (prefer local if path exists)
    ...
    def load_model(self):
        ...
        # Try BitsAndBytesConfig (may fail if bitsandbytes not installed/compatible)
        try:
            ...
            logger.warning(f"Could not create BitsAndBytesConfig: {e}. Will attempt non-quantized load.")
            bnb_config = None

        if bnb_config is not None:
            try:
                logger.info("Attempting to load model with 4-bit quantization (BitsAndBytes)...")
                ...
                logger.warning(f"Quantized model loading failed: {e}. Falling back to non-quantized load.")

        logger.info("Loading model without BitsAndBytes quantization (this will use more memory).")
        ...
        try:
            # prepare_model_for_kbit_training may not be applicable here and can be skipped
            self.model = prepare_model_for_kbit_training(self.model)
            self.model = self._apply_qlora()
        except Exception:
            logger.info("Skipping QLoRA preparation for non-quantized model.")

        logger.info("✓ Model loaded")
        return self.model

    def train(self, train_dataset, eval_dataset, num_epochs=3, learning_rate=1e-5, batch_size=4, grad_accum=8):
